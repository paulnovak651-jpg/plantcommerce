...
-- ============================================================================
-- Migration 050: Add alias_names column to material_search_index
--
-- The search_text column already includes normalized alias text for matching.
-- This migration adds a separate alias_names column that preserves the
-- display-friendly alias text (proper capitalization), enabling the
-- autocomplete route to show "also known as: Filbert" without a second
-- query to the aliases table.
--
-- Changes:
--   - Adds alias_names TEXT column (pipe-separated display names)
--   - Preserves all existing columns, behavior, and curation filters
...
SELECT
  ...
  a.alias_display_names AS alias_names,
  gp.usda_zone_min,
  gp.usda_zone_max,
...
LEFT JOIN LATERAL (
  SELECT
    string_agg(al.normalized_text, ' ') AS alias_list,
    string_agg(al.alias_text, '|' ORDER BY al.priority DESC, al.alias_text) AS alias_display_names
  FROM aliases al
...
-- Recreate all indexes from migration 033
