...
import type { AliasEntry, PipelineOutput, ResolutionStatus } from './types.js';
import { normalize, parseProductName } from './parser.js';

/**
 * Create a Supabase client with the service role key.
 * The service role key bypasses RLS, which is required for pipeline tables.
 */
 ...
 * This should be called once on startup and cached.
 */
export async function buildAliasIndexFromSupabase(
  ...
  for (const cv of cultivars ?? []) {
    // Map material_type to our EntityType
  ...
  // We need a lookup from UUID → canonical name + entity type
  const cultivarMap = new Map<string, { name: string; materialType: string }>();
  for (const cv of cultivars ?? []) {
    cultivarMap.set(cv.id, { name: cv.canonical_name, materialType: cv.material_type });
  }
  ...
  if (peErr) throw new Error(`Failed to load plant_entities: ${peErr.message}`);

  const peMap = new Map<string, { name: string; botanicalName: string | null }>();
  for (const pe of plantEntities ?? []) {
    peMap.set(pe.id, { name: pe.canonical_name, botanicalName: pe.botanical_name });

    // Index plant entity canonical names
...
export async function writePipelineResult(
  ...
  const { data: rawRow, error: rawErr } = await supabase
    ...
    .insert({
      ...
      parsed_organic: result.parsedOrganic === 'organic' ? true : result.parsedOrganic === 'conventional' ? false : null,
  ...
  if (rawErr) throw new Error(`Failed to write raw_inventory_row: ${rawErr.message}`);

  // 2. If resolved → create inventory_offer
  if (result.resolvedEntityType !== 'unresolved') {
    const isPlantEntity = result.resolutionStatus === 'resolved_plant_entity';
    const sourceOfferKey = generateSourceOfferKey(nurseryId, rawData.productPageUrl, rawData.rawSku, rawData.rawFormSize);
    ...
    const { data: offer, error: offerErr } = await supabase
      ...
      .insert({
        ...
        organic_status: result.parsedOrganic === 'organic' ? true : result.parsedOrganic === 'conventional' ? false : null,
        import_run_id: importRunId,
    ...
    if (offerErr) throw new Error(`Failed to write inventory_offer: ${offerErr.message}`);
  ...
  if (unmatchedErr) throw new Error(`Failed to write unmatched_name: ${unmatchedErr.message}`);

  return { unmatchedId: unmatched!.id, rawRowId: rawRow!.id };
}

/**
 * Create or get an import run.
 */
export async function createImportRun(
  ...
  if (error) throw new Error(`Failed to create import_run: ${error.message}`);
  return data!.id;
}

/**
 * Complete an import run with stats.
 */
export async function completeImportRun(
  ...
  if (error) throw new Error(`Failed to complete import_run: ${error.message}`);
}

// ── Helpers ──

function materialTypeToEntityType(materialType: string): AliasEntry['entityType'] {
  switch (materialType) {
    case 'cultivar_clone':
      return 'cultivar';
...
function generateSourceOfferKey(
  ...
  // Simple deterministic key: nursery + url + sku + form
